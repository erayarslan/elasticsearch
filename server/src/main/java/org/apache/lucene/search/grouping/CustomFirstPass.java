 ...
 * @lucene.experimental
 */
public class CustomFirstPass<T> extends SimpleCollector {
    ...
    public CustomFirstPass(GroupSelector<T> groupSelector, Sort groupSort, Sort topSort, int topNGroups) {
        ...
        compIDXEnd = leafComparators.length - 1;
        ...
        for (int i = 0; i < topSortFields.length; i++) {
            ...
            // use topNGroups + 1 so we have a spare slot to use for comparing (tracked by this.spareSlot):
            topComparators[i] = sortField.getComparator(topNGroups + 1, i);
    ...
    public Collection<SearchGroup<T>> getTopGroups(int groupOffset) throws IOException {
        ...
        for(CollectedSearchGroup<T> group : orderedGroups) {
            ...
            for(int sortFieldIDX=0;sortFieldIDX<sortFieldCount;sortFieldIDX++) {
                searchGroup.sortValues[sortFieldIDX] = topComparators[sortFieldIDX].value(group.comparatorSlot);
            }
        ...
        final Comparator<CollectedSearchGroup<?>> comparator = new Comparator<CollectedSearchGroup<?>>() {
            ...
            public int compare(CollectedSearchGroup<?> o1, CollectedSearchGroup<?> o2) {
                for (int compIDX = 0;; compIDX++) {
                    ...
                    final int c = topReversed[compIDX] * fc.compare(o1.comparatorSlot, o2.comparatorSlot);
                    if (c != 0) {
                        return c;
                    } else if (compIDX == topCompIDXEnd) {
