...
public class CollapseContext {
    ...
    public CollapsingTopDocsCollector<?> createTopDocs(IndexSearcher indexSearcher, Query query, Sort sort, int topN, FieldDoc after) {
        Sort collapseSort = this.sort == null ? null : this.sort.sort;

        if (fieldType.collapseType() == CollapseType.KEYWORD) {
            return CollapsingTopDocsCollector.createKeyword(indexSearcher, query, fieldName, fieldType, sort, collapseSort, topN, after);
        } else if (fieldType.collapseType() == CollapseType.NUMERIC) {
            return CollapsingTopDocsCollector.createNumeric(indexSearcher, query, fieldName, fieldType, sort, collapseSort, topN, after);
        } else {
